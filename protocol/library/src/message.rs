...
use crate::protocol::{
    GurtStatusCode, BODY_SEPARATOR, HEADER_SEPARATOR, PROTOCOL_PREFIX, GURT_VERSION,
};
use chrono::Utc;
...
impl GurtMethod {
    pub fn parse(s: &str) -> Result<Self> {
        match s.to_uppercase().as_str() {
            ...
            _ => Err(GurtError::invalid_message(format!(
                "Unsupported method: {}",
                s
            ))),
        }
...
impl GurtRequest {
    ...
    pub fn host(&self) -> Option<&str> {
        self.header("host").map(|s| s.as_str())
    }
    ...
    pub fn to_string(&self) -> String {
        ...
        if !headers.contains_key("user-agent") {
            headers.insert(
                ...
                format!("GURT-Client/{}", GURT_VERSION),
            );
        }
...
impl GurtMessage {
    ...
    pub fn parse_bytes(data: &[u8]) -> Result<Self> {
        ...
        let first_line =
            std::str::from_utf8(&data[..first_line_end]).map_err(|_| {
                GurtError::invalid_message("Invalid UTF-8 in first line")
            })?;
