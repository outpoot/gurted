// src/server.rs
use crate::crypto::{DomainCertConfig, TlsConfig, GURT_ALPN, TLS_VERSION};
use crate::message::{GurtMessage, GurtMethod, GurtRequest, GurtResponse};
use crate::protocol::{BODY_SEPARATOR, GurtStatusCode, MAX_MESSAGE_SIZE};
...
impl ServerContext {
    ...
    pub fn host(&self) -> Option<String> {
        self.request
            .header("host")
            .map(|h| extract_hostname(h).to_string())
    }
}
...
pub trait GurtHandler: Send + Sync {
    fn handle(
        &self,
        ctx: &ServerContext,
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<GurtResponse>> + Send + '_>>;
}
...
impl Route {
    ...
    pub fn matches_path(&self, path: &str) -> bool {
        ...
        self.path_pattern == path_without_query
            ...
            && path_without_query.starts_with(
...
impl HostMatcher {
    ...
    fn matches(&self, host: &str) -> bool {
        ...
        if let Some(rest) = pat.strip_prefix("*.") {
            // Match subdomains and optionally the root
...
struct HostedRoute {
    ...
    route: Route,
    handler: Arc<dyn GurtHandler>,
}

pub struct DomainCertFile {
...
impl GurtServer {
    ...
    pub fn with_tls_certificates_multi(configs: Vec<DomainCertFile>) -> Result<Self> {
        let mut server = Self::new();
        server.load_tls_certificates_multi(configs)?;
        Ok(server)
    }
    ...
    pub fn load_tls_certificates_multi(&mut self, configs: Vec<DomainCertFile>) -> Result<()> {
        info!("Loading multiple TLS certificates for SNI");

        if configs.is_empty() {
            return Err(GurtError::crypto(
                "No domain certificate configurations provided",
            ));
        }

        let mut domain_cfgs: Vec<DomainCertConfig> = Vec::new();

        for conf in configs {
            let cert_data = fs::read(&conf.cert_path).map_err(|e| {
                GurtError::crypto(format!(
                    "Failed to read certificate file '{}': {}",
                    conf.cert_path, e
                ))
            })?;
            let key_data = fs::read(&conf.key_path).map_err(|e| {
                GurtError::crypto(format!(
                    "Failed to read private key file '{}': {}",
                    conf.key_path, e
                ))
            ...
            let mut ccur = std::io::Cursor::new(cert_data);
            let certs: Vec<CertificateDer<'static>> =
                rustls_pemfile::certs(&mut ccur)
                    .collect::<std::result::Result<Vec<_>, _>>()
                    .map_err(|e| GurtError::crypto(format!("Failed to parse certificates: {}", e)))?;

            if certs.is_empty() {
                return Err(GurtError::crypto(format!(
                    "No certificates found in {}",
                    conf.cert_path
                )));
            }

            let mut kcur = std::io::Cursor::new(key_data);
            let private_key = rustls_pemfile::private_key(&mut kcur)
                .map_err(|e| GurtError::crypto(format!("Failed to parse private key: {}", e)))?
                .ok_or_else(|| {
                    GurtError::crypto(format!("No private key found in {}", conf.key_path))
                })?;

            domain_cfgs.push(DomainCertConfig {
                names: conf.names,
                cert_chain: certs,
        ...
        let tls_config = TlsConfig::new_server_multi(domain_cfgs)?;
        self.tls_acceptor = Some(tls_config.get_acceptor()?);

        info!("SNI TLS configuration loaded successfully");
        Ok(())
    }

    fn route_host<H>(mut self, host: Option<&str>, route: Route, handler: H) -> Self
    where
        H: GurtHandler + 'static,
    {
        let host_matcher = host.map(|h| HostMatcher::new(h.to_lowercase()));
        self.routes.push(HostedRoute {
            host: host_matcher,
            route,
    ...
    pub fn route<H>(self, route: Route, handler: H) -> Self
    where
        H: GurtHandler + 'static,
    {
        self.route_host(None, route, handler)
    }
    ...
    // Host-scoped route helpers

    pub fn get_host<F, Fut>(self, host: &str, path: &str, handler: F) -> Self
    where
        F: Fn(&ServerContext) -> Fut + Send + Sync + 'static,
        Fut: std::future::Future<Output = Result<GurtResponse>> + Send + 'static,
    {
        self.route_host(Some(host), Route::get(path), FnHandler { handler })
    }

    pub fn post_host<F, Fut>(self, host: &str, path: &str, handler: F) -> Self
    where
        F: Fn(&ServerContext) -> Fut + Send + Sync + 'static,
        Fut: std::future::Future<Output = Result<GurtResponse>> + Send + 'static,
    {
        self.route_host(Some(host), Route::post(path), FnHandler { handler })
    }

    pub fn put_host<F, Fut>(self, host: &str, path: &str, handler: F) -> Self
    where
        F: Fn(&ServerContext) -> Fut + Send + Sync + 'static,
        Fut: std::future::Future<Output = Result<GurtResponse>> + Send + 'static,
    {
        self.route_host(Some(host), Route::put(path), FnHandler { handler })
    }

    pub fn delete_host<F, Fut>(self, host: &str, path: &str, handler: F) -> Self
    where
        F: Fn(&ServerContext) -> Fut + Send + Sync + 'static,
        Fut: std::future::Future<Output = Result<GurtResponse>> + Send + 'static,
    {
        self.route_host(Some(host), Route::delete(path), FnHandler { handler })
    }

    pub fn head_host<F, Fut>(self, host: &str, path: &str, handler: F) -> Self
    where
        F: Fn(&ServerContext) -> Fut + Send + Sync + 'static,
        Fut: std::future::Future<Output = Result<GurtResponse>> + Send + 'static,
    {
        self.route_host(Some(host), Route::head(path), FnHandler { handler })
    }

    pub fn options_host<F, Fut>(self, host: &str, path: &str, handler: F) -> Self
    where
        F: Fn(&ServerContext) -> Fut + Send + Sync + 'static,
        Fut: std::future::Future<Output = Result<GurtResponse>> + Send + 'static,
    {
        self.route_host(Some(host), Route::options(path), FnHandler { handler })
    }

    pub fn patch_host<F, Fut>(self, host: &str, path: &str, handler: F) -> Self
    where
        F: Fn(&ServerContext) -> Fut + Send + Sync + 'static,
        Fut: std::future::Future<Output = Result<GurtResponse>> + Send + 'static,
    {
        self.route_host(Some(host), Route::patch(path), FnHandler { handler })
    }

    pub fn any_host<F, Fut>(self, host: &str, path: &str, handler: F) -> Self
    where
        F: Fn(&ServerContext) -> Fut + Send + Sync + 'static,
        Fut: std::future::Future<Output = Result<GurtResponse>> + Send + 'static,
    {
        self.route_host(Some(host), Route::any(path), FnHandler { handler })
    }
    ...
    async fn handle_connection(&self, mut stream: TcpStream, addr: SocketAddr) -> Result<()> {
        ...
        if let Some(tls_acceptor) = &self.tls_acceptor {
            ...
            let tls_stream = tls_acceptor.accept(stream).await.map_err(|e| {
                GurtError::crypto(format!("TLS upgrade failed: {}", e))
            })?;
            ...
            Err(GurtError::protocol(
                "TLS is required after handshake but no TLS configuration available",
            ))
        }
    }

    async fn handle_initial_handshake(
        &self,
        ...
        match message {
            GurtMessage::Request(request) => {
                if request.method == GurtMethod::HANDSHAKE {
                    self.send_handshake_response(stream, addr, &request)
                        .await
                } else {
                    Err(GurtError::protocol(
            ...
            GurtMessage::Response(_) => {
                Err(GurtError::protocol(
                    "Server received response during handshake",
                ))
            }
        }
    }
    ...
    async fn handle_tls_connection(
        ...
        loop {
            let bytes_read = match tls_stream.read(&mut temp_buffer).await {
                ...
                Err(e) => {
                    if e.kind() == std::io::ErrorKind::UnexpectedEof {
                        debug!(
                            "Client {} closed connection without TLS close_notify (benign)",
                            addr
                        );
                        break;
                    }
            ...
            let body_separator = BODY_SEPARATOR.as_bytes();
            let has_complete_message = buffer
                .windows(body_separator.len())
                .any(|w| w == body_separator)
            ...
            if has_complete_message {
                match self.process_tls_message(&mut tls_stream, addr, &buffer).await {
                    ...
                    Err(e) => {
                        error!(
                            "Encrypted message processing error from {}: {}",
                            addr, e
                        );
    ...
    async fn send_handshake_response(
        ...
        info!(
            "Handshake response sent to {}, preparing for TLS upgrade",
            addr
        );
        Ok(())
    }
    ...
    async fn process_tls_message(
        ...
        match message {
            GurtMessage::Request(request) => {
                if request.method == GurtMethod::HANDSHAKE {
                    Err(GurtError::protocol(
                        "Received HANDSHAKE over TLS - protocol violation",
                    ))
                } else {
                    self.handle_encrypted_request(tls_stream, addr, &request)
                        .await
                }
            }
            GurtMessage::Response(_) => {
    ...
    async fn handle_default_options(
        &self,
        tls_stream: &mut TlsStream<TcpStream>,
        request: &GurtRequest,
    ) -> Result<()> {
        ...
        let req_host = request
            .header("host")
            .map(|h| extract_hostname(h))
            .unwrap_or("");

        for hosted in &self.routes {
            let host_match = match &hosted.host {
                Some(hm) => hm.matches(req_host),
                None => true,
            };
            if host_match && hosted.route.matches_path(&request.path) {
                if let Some(method) = &hosted.route.method {
    ...
    async fn handle_default_head(
        ...
        for hosted in &self.routes {
            ...
            if host_match
                && hosted.route.method == Some(GurtMethod::GET)
                && hosted
                .route
                .matches(&GurtMethod::GET, &request.path)
            {
                ...
                match hosted.handler.handle(&context).await {
                    Ok(mut response) => {
                        let original_content_length = response.body.len();
                        response.body.clear();
                        response =
                            response.with_header("content-length", original_content_length.to_string());
                    ...
                    Err(e) => {
                        error!(
                            "Handler error for HEAD {} (via GET): {}",
                            request.path, e
                        );
                        let error_response = GurtResponse::internal_server_error();
    ...
    async fn handle_encrypted_request(
        ...
        debug!(
            "Handling encrypted {} request to {} from {}",
        ...
        debug!(
            "Incoming request from {}: method={} path={} host={}",
            addr, request.method, request.path, req_host
        );

        // Try HOST-specific routes first
        for hosted in &self.routes {
            if let Some(hm) = &hosted.host {
                if hm.matches(&req_host) && hosted.route.matches(&request.method, &request.path) {
                    return self.invoke_handler(tls_stream, addr, request.clone(), hosted).await;
                }
            }
        }

        // Fall back to global (no-host) routes
        for hosted in &self.routes {
            if hosted.host.is_none() && hosted.route.matches(&request.method, &request.path) {
                return self.invoke_handler(tls_stream, addr, request.clone(), hosted).await;
            }
        }

        match request.method {
            GurtMethod::OPTIONS => self.handle_default_options(tls_stream, request).await,
            GurtMethod::HEAD => self.handle_default_head(tls_stream, addr, request).await,
            _ => {
                let not_found_response =
                    GurtResponse::not_found().with_string_body("Not found");
                tls_stream.write_all(&not_found_response.to_bytes()).await?;
                Ok(())
            }
        }
...
mod tests {
    ...
    fn test_host_matcher() {
        let m1 = HostMatcher::new("example.com".to_string());
        assert!(m1.matches("example.com"));
        assert!(!m1.matches("www.example.com"));

        let m2 = HostMatcher::new("*.example.com".to_string());
        assert!(m2.matches("www.example.com"));
        assert!(m2.matches("api.example.com"));
        assert!(m2.matches("example.com"));

        let m3 = HostMatcher::new("*".to_string());
        assert!(m3.matches("anything.local"));
    }

    #[test]
    fn test_extract_hostname() {
        assert_eq!(extract_hostname("example.com:4878"), "example.com");
        assert_eq!(extract_hostname("[::1]:4878"), "::1");
